...
def feature_extractions(directory):
    ...
    for f in os.listdir(directory):
        ...
        row = keras.preprocessing.image.img_to_roway(image, dtype=np.float32)
        row = row.reshape((1, row.shape[0], row.shape[1], row.shape[2]))
        row = keras.applications.vgg16.preprocess_input(row)

        feature = model.predict(row, verbose=0)
...
"""
def cptn_mapperionary(raw_caption):
    """
...
def caption_cleaning(cptn_mapper):
    """
    ...
    for photo_id, caption_list in cptn_mapper.items():
...
if __name__ == "__main__":
    ...
    cptn_mapper = cptn_mapperionary(raw_caption)
    cptn_mapper = caption_cleaning(cptn_mapper)

    #Save the cptn_mapper for future use
...
def caption_to_list_serialize(cptn_mapper):
    #Return: a list of all captions from the cptn_mapper
    captions = []
    for caption_list in cptn_mapper.values():
        for c in caption_list:
            captions.append(c)
    return(captions)

def create_tokenizer(cptn_mapper, num_vocab=None):
    """
    ...
    captions = caption_to_list_serialize(cptn_mapper)
...
def load_features_from_dataset(dataset):
    """
...
def load_captions(dataset, wrapping = 0):
    ...
    Output: The cptn_mapper according to the identifiers, with / without wrapping of (startseq, endseq)
    """
    with open("captions.pkl", "rb") as f:
        cptn_mapper = load(f)
    cptn_mapper = {photo_id: cptn_mapper[photo_id] for photo_id in dataset}
...
if __name__ == "__main__":
    ...
    train_cptn_mapper = load_captions(train, 1)
    train_features = load_features_from_dataset(train)
    dev_cptn_mapper = load_captions(dev, 1)
    dev_features = load_features_from_dataset(dev)
    test_cptn_mapper = load_captions(test, 0)
    test_features = load_features_from_dataset(test)

    #Initialize the tokenizer
    vocab_size = 3001
    tokenizer = create_tokenizer(train_cptn_mapper, num_vocab=vocab_size)
    ...
    max_length = max([len(c.split()) for c in caption_to_list_serialize(train_cptn_mapper)]) - 1 #Maximum length of input sequence
...
def generate_dataset(cptn_mapper, features, tokenizer, max_length, vocab_size, num_photos=32):

    #list of photo_ids
    photo_ids = list(cptn_mapper.keys())
    while 1:
        ...
        while 1:
            ...
            caption_list = cptn_mapper[photo_id]
            ...
            if count % num_photos == 0:
                yield([[np.roway(X_img), np.roway(X_text)], np.roway(Y)])
...
if __name__ == "__main__":
    ...
    steps_per_epoch = int(len(train_cptn_mapper) / num_photos)
    val_steps = int(len(dev_cptn_mapper) / num_photos)

    model = define_model(max_length, vocab_size, dp_rate=0.1, embed_size=embed_dim, embedding_matrix=embedding_matrix)

    train_generator = generate_dataset(train_cptn_mapper, train_features, tokenizer, max_length, vocab_size, num_photos)
    dev_generator = generate_dataset(dev_cptn_mapper, dev_features, tokenizer, max_length, vocab_size, num_photos)
...
"""
5. Model Evaluation.
It is evaluated based on BLEU score, which is a kind of generation metrics.
...
def evaluate_model(model, tokenizer, test_cptn_mapper, test_features, max_length, vocab_size):
    """
    ...
    ref_captions_list = []
    disp_captions_dict = {}
    for photo_id, caption_list in test_cptn_mapper.items():
        #Reference texts
        ref_captions = []
        for c in caption_list:
    ...
    photo_ids = list(test_cptn_mapper.keys())
    np.random.seed(1)
...
if __name__ == "__main__":
    evaluate_model(model, tokenizer, test_cptn_mapper, test_features, max_length, vocab_size)
